...
class DBHelper:
    ...
    def fetch_otheruserdata(self, user_id, gender):

        self._mycursor.execute("SELECT * FROM users WHERE user_id NOT LIKE '{}' AND gender NOT LIKE '{}' ".format(user_id,gender))
        data=self._mycursor.fetchall()
    ...
    def register(self,name,email,password,age,gender,city,dp):
        try:
            self._mycursor.execute("""INSERT INTO users (user_id,name,email,password,age,gender,city,dp)
                                  VALUES
                                  (NULL,'{}','{}','{}',{},'{}','{}','{}')"""
    ...
    def update_info(self,password,age,city,dp,user_id):
        try:
            self._mycursor.execute("""
                           UPDATE users SET password='{}', age={}, city='{}', dp='{}'
                           WHERE user_id LIKE {}
                           """.format(password,age,city,dp,user_id))
            self._conn.commit()
            return 1
        except:
            return 0
